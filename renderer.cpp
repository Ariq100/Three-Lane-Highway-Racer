...
void draw_enemies(GameState& state)
{
    for (int i = 0; i < state.active_enemies.length(); i++)
    {
        ...
        switch (e.type)
        {
            case TRUCK:       bmp = truck_bitmap; break;
            case MINIVAN:     bmp = minivan_bitmap; break;
            case SEDAN:       bmp = sedan_bitmap; break;
            case RECKLESS_CAR:bmp = reckless_bitmap; break;
            default:          bmp = sedan_bitmap; break;
        }
